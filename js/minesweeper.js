    ...
    $.fn.minesweeperjs = function(options) {
        return this.each(function() {
            ...
            */
            ...
            /**
            * Returns the number of neighboring cells that contain a mine.
            */
            function getNeighboringMines(cell) {
                var mines = 0;
                var neighbors = getCellNeighbors(cell);
                                
                for (var i = 0; i < neighbors.length; i++) {
                    if (neighbors[i].data("mine")) {
                        mines++;
                    }
                }
                return mines;
            ...
            */
            function stepOnCell(cell) {
                ...
                // Cell contains no mine and neigboring cells are checked for mines
            ...
            /**
            * Checks a cell for the number of neighboring mines (recursively).
            */
            function checkCell(cell) {
                var mines = getNeighboringMines(cell);
                ...
                if (mines == 0) {
                    var neighbors = getCellNeighbors(cell);
                    for (var i = 0; i < neighbors.length; i++) {
                        if (!neighbors[i].data("checked")) {
                            checkCell(neighbors[i]);
                        }
                    }
                }
                else {
            ...
            /**
            * Returns all neighboring cells of a certain cell.
            */
            function getCellNeighbors(cell) {
                var neighbors = new Array();
                                
                if (cell.data("x") != 0) {
                    neighbors.push($("#" + (cell.data("x") - 1) + "-" + cell.data("y")));
                }
                if (cell.data("x") != defaults.grid.width - 1) {
                    neighbors.push($("#" + (cell.data("x") + 1) + "-" + cell.data("y")));
                }
                if (cell.data("y") != 0) {
                    neighbors.push($("#" + cell.data("x") + "-" + (cell.data("y") - 1)));
                }
                if (cell.data("y") != defaults.grid.height - 1) {
                    neighbors.push($("#" + cell.data("x") + "-" + (cell.data("y") + 1)));
                }
                if (cell.data("x") != 0 && cell.data("y") != 0) {
                    neighbors.push($("#" + (cell.data("x") - 1) + "-" + (cell.data("y") - 1)));
                }
